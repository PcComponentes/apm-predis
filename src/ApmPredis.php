...
class ApmPredis implements ClientInterface
{
    ...
    private const CONTEXT_DB_TYPE = 'redis';
    ...
    {
        return $this->client->createCommand($method, $arguments);
    }
     ...
     * @throws Exception
     */
    private function startSpan(string $commandID, array $arguments): Span
    {
        return $this->elasticApmTracer->startSpan(
            (new ApmSpanNameContext($commandID))->generateName($commandID, $arguments),
            self::SPAN_TYPE,
            self::SPAN_SUBTYPE,
            $commandID,
            Context::fromDb(
                new Db(
                    $this->instance,
                    null,
                    (new ApmStatementContext($commandID))->generateStatement($arguments),
    ...
    private function callWithApm(string $commandID, array $arguments)
    {
        $span = $this->startSpan($commandID, $arguments);
        $response = $this->client->__call($commandID, $arguments);
        $span->stop();
